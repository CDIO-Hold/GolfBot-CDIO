...
def detect_robot(robot, x1, x2, y1, y2) -> Robot:
...
def detect_robot_front(robot, x1, x2, y1, y2):
    x = (x1 + x2) / 2
    y = (y1 + y2) / 2
...
def detect_goal(name, x1, x2, y1, y2) -> Goal:
    # Center coords
    x = (x1 + x2) / 2
    y = (y1 + y2) / 2
...
def detect_egg(class_name, x1, x2, y1, y2) -> Egg:
    x = (x1 + x2) / 2
    y = (y1 + y2) / 2
    ...
    for r in result:
        ...
        if current_class == "orange-ball" or current_class == "white-ball":
            ...
            if current_wall.is_left_wall or current_wall.is_right_wall:
                ...
                cvzone.putTextRect(img, text, (max(0, goal.position.x), max(35, goal.position.y)), scale=1, thickness=1)


        elif current_class == "robot":
            robot = detect_robot(robot, x1, x2, y1, y2)

        elif current_class == "robot-front":
            if robot is not None:
                robot = detect_robot_front(robot, x1, x2, y1, y2)
                text = f'{current_class} {confidence:.2f}% angle={robot.facing.angle:.2f}'
                cvzone.putTextRect(img, text, (max(0, x1), max(35, y1)), scale=1, thickness=1)
        elif current_class == "egg":
            egg = detect_egg(current_class, x1, x2, y1, y2)
